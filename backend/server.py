...
def build_pdf_report(metrics, scan):
    ...
    for lbl, w in headers:
        pdf.cell(w, 7, f"  {lbl.upper()}", fill=True, border='B')
    pdf.ln()
    ...
    for i, f in enumerate(scan['findings']):
        ...
        pdf.set_text_color(*pdf.C_TEXT)
        pdf.cell(32, 6.5, f"  {f['category']}", fill=True)
        pdf.cell(58, 6.5, f"  {f['name']}", fill=True)
        pdf.set_font("Helvetica", "B", 8)
        pdf.set_text_color(*sc)
        pdf.cell(20, 6.5, f"  {f['status']}", fill=True)
        ...
        if len(details) > 62:
            details = details[:62] + "..."
        pdf.cell(70, 6.5, f"  {details}", fill=True, new_x="LMARGIN", new_y="NEXT")
        ...
        if f.get('recommendation'):
            ...
            pdf.cell(0, 5, f"  -> Recommendation: {f['recommendation']}", fill=True, new_x="LMARGIN", new_y="NEXT")
    ...
    if recs:
        ...
        for j, (name, rec) in enumerate(recs):
            ...
            pdf.cell(0, 6, f"  {rec_disp}", fill=True, new_x="LMARGIN", new_y="NEXT")
...
if __name__ == "__main__":
    print("=" * 50)
    print("  SysMon Pro - System Monitor & Security Scanner")
    print("=" * 50)
