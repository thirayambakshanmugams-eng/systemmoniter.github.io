...
def get_all_latest_metrics_for_user(user_id):
    """Get the latest metrics for ALL devices belonging to a user."""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('''
        SELECT lm.*, d.device_name, d.hostname, d.os_info, d.last_seen,
               d.registered_at, d.user_id
        FROM latest_metrics lm
        JOIN devices d ON d.device_id = lm.device_id
        WHERE d.user_id = ?
        ORDER BY lm.timestamp DESC
